...
std::string standard_introduction_text = "Welcome to the Semantic DB version 4\n";
...
int main(int argc, char** argv) {
    ...
    try {
        TCLAP::CmdLine cmd("The Semantic DB 4 shell", ' ', "4");
        ...
        std::cout << interactive_introduction_text;
        {
            ...
            timer.Stop();
            if(!quiet_mode && !file_names.empty()) {
                std::cout << "\n    Time taken" << display_time(timer.GetTime()) << std::endl;
            }
        }
        ...
        std::vector<std::string> indent_line_tokens{ "if(" , "for(", "sfor(", "while(" };
