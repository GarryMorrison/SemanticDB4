...
{
    if (parameters.size() != 4) { return Ket(); }
    if (parameters[0]->type() != CINT || parameters[1]->type() != CINT || parameters[2]->type() != CINT || parameters[3]->type() != COPERATOR) { return Ket(); }
    int patch_count = 0;  // Number of patches generated by this operator.
    int seq_len = (int)(input_seq.size());
    int input_width = parameters[0]->get_int();
    int input_height = seq_len / input_width;
    if (seq_len % input_width != 0) { input_height++; }
    int output_width = parameters[1]->get_int();
    int output_height = parameters[2]->get_int();
    if (input_width <= 0 || input_height <= 0 || output_width <= 0 || output_height <= 0) { return Ket(); }
    if (output_width > input_width || output_height > input_height) { return Ket(); }
    SimpleOperator op = parameters[3]->get_operator();
