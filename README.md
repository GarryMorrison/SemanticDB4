# Semantic DB 4
A project to add a graphical user interface to the [Semantic DB](https://github.com/GarryMorrison/SemanticDB3.1). The GUI component is written in the cross-platform GUI library [wxWidgets](https://www.wxwidgets.org), and on Windows, the parser uses [winflexbison](https://github.com/lexxmark/winflexbison). Graphing uses [Graphviz](https://graphviz.org/).

The project is almost ready for official release. The GUI has added a lot of power that wasn't present in the command line version, including most things are now clickable or shift-clickable. If you click on a literal operator and/or a ket, then relevant knowledge will be displayed. If you click on an operator, then the relevant usage info window will pop-up. Also, we have two main ways of entering knowledge. Either the [command window](https://github.com/GarryMorrison/SemanticDB4/blob/master/documentation-templates/html/images/SDB-GUI--command-window.png), useful for when you want to test an idea quickly, or the [edit window](https://github.com/GarryMorrison/SemanticDB4/blob/master/documentation-templates/html/images/SDB-GUI--edit-window.png) when you want to type out some code with minimal distractions.

There is a brief (but outdated) description of the graphical user interface on the [website](http://semantic-db.org/gui/index.html), including some screenshots. And we recently added "sw experiments" also described on the [website](http://semantic-db.org/gui/sw-experiments.html).

The code
--------

The code has been factored into a core library, a command line interface, and the primary graphical interface. If you just want to run a script and see the output, the command line interface is sometimes more efficient. We now have [Ubuntu build instructions](https://github.com/GarryMorrison/SemanticDB4/blob/master/ubuntu-build-instructions.md), with more to be added later. For windows it is more involved, since you will need to install and configure both wxWidgets and winflexbison, then compile using the Visual Studio solution. It is simpler to run the .exe available in [the releases](https://github.com/GarryMorrison/SemanticDB4/releases).

The operator documentation
--------------------------

The documentation for our operators can be generated from the GUI under the menu: `Usage -> Generate docs ...`, or is available included with the [latest release](https://github.com/GarryMorrison/SemanticDB4/releases), or finally on [the website](http://semantic-db.org/docs/usage4/index.html). Our code uses a [template system](https://github.com/GarryMorrison/SemanticDB4/tree/master/documentation-templates) of sorts for the documentation generation, so that with minimal work it can generate different documentation types. Currently though we only have a couple of variations of html.
...
Using the Semantic DB
---------------------

Once the main project has been compiled (see for example the [Ubuntu case](https://github.com/GarryMorrison/SemanticDB4/blob/master/ubuntu-build-instructions.md)), we interact with the system using either the [GUI](https://github.com/GarryMorrison/SemanticDB4/blob/master/documentation-templates/html/images/SDB-GUI--command-window.png) or the command line interface. This is where we load, type, interact and then save knowledge. In windows run the `SemanticDB4.exe` available in the [release section](https://github.com/GarryMorrison/SemanticDB4/releases). On Linux either invoke the GUI or the CLI versions using `./SDB4_GUI &` or `./SDB4_CLI`. Invoke on the command line with `./SDB4_CLI --help` to see brief usage information. Invoke with `-i` or `--interactive` to enter the interactive agent. Once in the agent, type `help` to see the inline help message. Type `usage` to see a list of operators available in our operator library. Type `usage op` to see the specific usage information for the specified operator (note that currently a small number of operators do not have usage information defined, but that will slowly improve).

---

Brief [usage information](http://semantic-db.org/docs/usage4/index.html) for our operators
